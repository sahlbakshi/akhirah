...
export default function Home() {
  ...
  return (
    <div className="flex flex-col items-center sm:mt-28 mt-10">
      <div className="text-2xl sm:text-3xl font-bold mb-28">AKHIRAH ATHLETIC</div>
      <div className={`text-[20px] sm:text-[28px] font-semibold ${message != "" ? "mb-10" : "mb-14"}`}>WAITLIST FOR OUR DROP</div>
      {message != "" ? <div className="w-64 text-center bg-red-100 text-red-900 font-semibold py-6">{message}</div> : null}
      <form className="flex flex-col items-center">
        <input className="outline-none text-center w-64 bg-black py-4 border-b border-gray-500 font-medium placeholder:font-medium placeholder:text-gray-300 mb-8"
          type="email"
          value={email}
